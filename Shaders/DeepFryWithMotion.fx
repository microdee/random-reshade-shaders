...
uniform float MotionPower<
	ui_type = "slider";
	ui_min = 0; ui_max = 4;
> = 1;

uniform float MotionBrightnessThreshold<
	ui_type = "slider";
	ui_min = 0; ui_max = 1;
> = 0.741;
...
void VerticalPS0(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 output : SV_Target0)
{
	...
	float2 vel = tex2Dfetch(sMotionVectorTex, vpos.xy).xy;
	float modulate = pow(length(vel), MotionPower) * (length(color) > MotionBrightnessThreshold);
	float quality = Quality + modulate * Motion;
