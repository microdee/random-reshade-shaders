...
on top of the current color buffer.
...
float4 BlendImpl(sampler previousSampler, float4 pixelPos)
{
	uint2 pixelCoord = uint2(pixelPos.xy);
	float4 left, right;
	if (BlendOrder > 0)
	{
		left = tex2Dfetch(previousSampler, pixelCoord);
		right = tex2Dfetch(ReShade::BackBuffer, pixelCoord);
	}
	else
	{
		left = tex2Dfetch(ReShade::BackBuffer, pixelCoord);
		right = tex2Dfetch(previousSampler, pixelCoord);
	}
...
#define BLEND_PREVIOUS(id) \
 \
namespace BlendPrevious \
{ \
	texture BlendTex_##id { Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = RGBA16F; MipLevels = 1; }; \
	sampler Sampler { Texture = BlendTex_##id; }; \
} \
 \
float4 PS_Blend(float4 pixelPos : SV_Position) : SV_Target \
{ \
	return BlendImpl(BlendPrevious::Sampler, pixelPos); \
} \
 \
technique BlendPrevious_Write_##id \
{ \
	pass MainPass \
	{ \
		VertexShader = VS; \
		PixelShader = PS_Write; \
		RenderTarget = BlendPrevious::BlendTex_##id; \
		GenerateMipMaps = false; \
	} \
} \
 \
technique BlendPrevious_Blend_##id \
{ \
	pass MainPass \
	{ \
		VertexShader = VS; \
